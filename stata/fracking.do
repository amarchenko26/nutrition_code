...
// Our regression was "Income increases by $10000, how much does HI increase?"
// So rescale to 10,000s of dollars
cap drop d_irsagi_capita_k d_wages_capita_k d_irswages_capita_k

g d_wages_capita_k = d_wages_capita / 10000
g d_irswages_capita_k = d_irswages_capita / 10000
g d_irsagi_capita_k = d_irsagi_capita / 10000 
g d_irsotherinc_capita_k = d_irsotherinc_capita / 10000

// Other variables are scaled by lagged employment? 
gg hi_emp = hi / L.employment
...
// beta = .028, p = .041
ivreghdfe hi_capita (d_wages_capita_k = newvalue_capita_ins) if sample == 1, ///
	absorb(yeardum* fips) cluster($cluster_county) 

// first differences
ivreghdfe D.hi_capita (D.d_wages_capita_k = D.newvalue_capita_ins) if sample ==1, ///
	cluster($cluster_county) absorb(yeardum*)


cap drop f_hi
g f_hi = F.hi_capita - hi_capita
	
// fwd differences
ivreghdfe f_hi (D.d_wages_capita_k = D.newvalue_capita_ins) if sample ==1, ///
	cluster($cluster_county) absorb(yeardum*)

	

***************** OTHER CONTEMPORANEOUS WAGE AND INCOME OUTCOMES 

// beta = .063, p = 0.048
ivreghdfe hi_emp (d_irswages_capita_k = newvalue_capita_ins), absorb(yeardum* fips) cluster($cluster_county) 

// beta = .014, p = .103
ivreghdfe hi_emp (d_irsagi_capita_k = newvalue_capita_ins), absorb(yeardum* fips) cluster($cluster_county) 

// beta = .0179, p = .131
ivreghdfe hi_emp (d_irsotherinc_capita_k = newvalue_capita_ins), absorb(yeardum* fips) cluster($cluster_county) 
